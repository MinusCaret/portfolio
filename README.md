# Portfolio
## About me
Quality Assurance professional with 4 years of experience in video game and mobile QA, now expanding into SaaS testing. Experienced in manual testing, bug triage, and regression testing, with growing skills in automation using Playwright and API testing with Postman.

## Experience
Quality Assurance Analyst - Deep Field Games (July 2022 - Present)\
Testing and QC Supervisor - Kiwa Digital (September 2023 - April 2025)

- Manual QA for live software products
- Regression testing, bug reporting, and release validation
- Collaborating with developers and product teams to maintain quality standards

## Resume
Available as a PDF on [Google Drive](https://docs.google.com/document/d/1CMcMWKQnhmXB4X84P71F-kM2YLhaRH1K/edit?usp=sharing&ouid=116088990235051023940&rtpof=true&sd=true).

## Skills & Tools
- Manual QA: test case execution, regression testing, bug triage
- Automation & API: Playwright, Postman
- Version control & CI/CD: Git, GitHub, GitHub Actions
- Collaboration: Trello, Confluence, Slack, Google Drive

## Projects
- [UI (Playwright) tests for expandtesting.com](https://github.com/MinusCaret/expand-testing.git)
- [API (Postman) tests for expandtesting.com](https://github.com/MinusCaret/expand-testing-postman.git)
